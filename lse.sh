...
lse_run_tests_filesystem() {
  ...
  lse_test "fs110" "1" "Other interesting files in home directories" "`for h in $(cut -d: -f6 /etc/passwd); do find "$h" \( -name "*.rhosts" -o -name ".git-credentials" -o -name ".*history" \) -maxdepth 1 -exec ls -la {} 2>/dev/null \; ; done 2>/dev/null`"
